...
(function () {
    ...
    function calcForTeam(startersData, belongsToTeam) {
      for (let q = 1; q <= maxQ; q++) {
        ...
        qSubs.forEach((sub) => {
          ...
          const { outId: rawOut, inId: rawIn } = normalizeSub(sub);
          const pIn = parseId(rawIn);
          const pOut = parseId(rawOut);
  ...
  function recalcFullGame(actions, homePlayers, oppPlayers) {
    ...
    actions.forEach((rawAct) => {
      ...
      if (type === 'SHOT' && act.made && act.hockeyAssistId) {
        const hPid = isOpponent(act.hockeyAssistId)
          ? act.hockeyAssistId
          : parseInt(act.hockeyAssistId);
  ...
  function exportMatchLogsCSV(actions, players, oppPlayers) {
    ...
    sorted.forEach((a) => {
      ...
      if (a.type === 'SHOT') {
        ...
        desc = 'Changement';
        var _s = normalizeSub(a);
        details = `Entree ${getPName(_s.inId)} / Sortie ${getPName(_s.outId)}`;
        ...
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-600 max-w-3xl w-full mx-4 max-h-[90vh] flex flex-col">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                className="text-slate-400 hover:text-white cursor-pointer transition-colors duration-200 p-1 rounded"
                onClick={onClose}
              >
                <svg
  ...
  function ActionCard({ action, playerMap, onEdit, onDelete }) {
    ...
    if (action.type === 'FT') {
      icon = '🎯';
      label = 'LF ' + (action.ftMade || 0) + '/' + (action.ftAtt || 0);
    }
    if (action.type === 'SUB') {
      icon = '🔁';
      var _sub = normalizeSub(action);
      var inP  = playerMap[_sub.inId];
      var outP = playerMap[_sub.outId];
      label = (inP ? '#' + inP.number : '?') + ' ← ' + (outP ? '#' + outP.number : '?');
    ...
    if (action.type === 'TOV') {
      icon = '💨';
      label = action.unforced ? 'Perte NF' : 'Perte';
    }
    if (action.type === 'OREB') {
      icon = '🔄';
      label = 'Reb Off';
    ...
    if (action.type === 'STL') {
      ...
      label =
        'Interception' +
        (action.victim && playerMap[action.victim]
          ? ' / perte #' + playerMap[action.victim].number
          : '');
    }
    if (action.type === 'BLK') {
      icon = '✋';
    ...
    if (action.type === 'TIMEOUT') {
      icon = '⏸';
      label = 'Timeout ' + (action.team === 'home' ? 'Nous' : 'Eux');
      bgClass = 'bg-indigo-950/30';
      borderClass = 'border-l-indigo-500';
    }
    if (action.type === 'STOPPAGE') {
      icon = '⏱';
      label = 'Arrêt ' + (action.duration || 0) + 's';
      bgClass = 'bg-slate-800/50';
      borderClass = 'border-l-slate-400';
    }
    if (action.type === 'PAINT_TOUCH') {
      icon = '🎨';
      label = 'Paint Touch';
    }
    if (action.type === 'DEFLECTION') {
      icon = '👋';
      label = 'Déviation';
    }
    if (action.type === 'BOXOUT') {
      icon = '🧱';
      label = 'Box-out';
    }
    if (action.type === 'BLOWBY') {
      icon = '💨';
      label = 'Battu 1c1';
    ...
    return (
      <div
        className={
          'flex items-center gap-2 px-3 py-2 rounded border-l-2 mb-0.5 group cursor-pointer hover:bg-slate-800 transition-colors ' +
          bgClass +
          ' ' +
          borderClass
        }
      ...
      >
        ...
        <span
          className={
            'font-bold text-xs shrink-0 w-16 truncate ' + (isOpp ? 'text-red-400' : 'text-blue-400')
          }
        >
          {p ? '#' + p.number : ''}
        </span>
        <span className="text-slate-300 text-xs flex-1 truncate">
          ...
          {action.type === 'SHOT' && action.shotQuality && (
            <span
              className={
                'ml-1 text-[9px] font-bold px-1 rounded border ' +
                (action.shotQuality === 'open'
                  ? 'text-green-400 border-green-700'
                  : action.shotQuality === 'forced'
          ...
          {action.type === 'SHOT' && action.made && action.astId && playerMap[action.astId] && (
            <span className="text-purple-400 text-[9px] ml-1">
              ast #{playerMap[action.astId].number}
            </span>
          )}
          {action.type === 'SHOT' && action.hockeyAssistId && playerMap[action.hockeyAssistId] && (
            <span className="text-teal-400 text-[9px] ml-1">
              🏒#{playerMap[action.hockeyAssistId].number}
            </span>
          )}
          {action.type === 'FOUL' &&
            action.foulType === 'TECHNICAL' &&
            action.ftShooterId &&
            playerMap[action.ftShooterId] && (
              <span className="text-yellow-400 text-[9px] ml-1">
                LF:#{playerMap[action.ftShooterId].number}
              </span>
            )}
          {action.play && <span className="text-slate-600 text-[9px] ml-1">📋{action.play}</span>}
        </span>
        <button
          className="opacity-0 group-hover:opacity-100 text-red-400 text-xs px-1 transition-opacity"
          onClick={(e) => {
            e.stopPropagation();
            onDelete(action.id);
          }}
  ...
  function FilterBar({
    ...
    var hasFilter =
      filterQ > 0 || filterPid !== 'all' || filterType !== 'all' || filterPlay !== 'all';
    return (
      <div
        className="bg-slate-900 border-b border-slate-700 px-3 py-2 flex items-center gap-2 overflow-x-auto shrink-0"
      ...
      >
        ...
        <select
          value={filterPid}
          onChange={(e) => onFilterPid(e.target.value)}
        ...
        {playOptions.length > 0 && (
          <select
            value={filterPlay}
            onChange={(e) => onFilterPlay(e.target.value)}
            className="bg-slate-800 border border-slate-600 text-white text-[10px] rounded px-2 py-1 font-bold max-w-[100px] shrink-0"
          >
  ...
  function EditPanel({ action, players, oppPlayers, quarters, onSave, onDelete, onCancel }) {
    ...
    const [data, setData] = useState(() => {
      ...
      if (action.type === 'SUB') {
        const _s = normalizeSub(action);
        base.pid    = _s.inId;
        base.subOut = _s.outId;
      }
      return base;
    });
    const [showZone, setShowZone] = useState(false);
    return (
      <div>
        {showZone && (
          <ZoneSelector
            onSelect={(z) => {
              setData((d) => ({ ...d, val: z.val, zoneX: z.x, zoneY: z.y }));
              setShowZone(false);
            }}
            onCancel={() => setShowZone(false)}
          />
        ...
        <div
          className="fixed inset-x-0 bottom-0 z-50 bg-slate-800 border-t-2 border-orange-500 rounded-t-xl shadow-2xl max-h-[70vh] overflow-y-auto p-4"
        ...
        >
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <button onClick={onCancel} className="text-slate-400 text-lg leading-none">
              ✕
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <select
                ...
                value={data.type}
                onChange={(e) =>
                  setData((d) => ({ ...d, type: e.target.value, victim: null, subOut: null }))
                }
              >
                {ACTION_TYPES.map((t) => (
                  <option key={t.value} value={t.value}>
            ...
            <div>
              ...
              <div className="flex gap-1 items-center">
                <input
                  ...
                  value={data.timeMin}
                  onChange={(e) =>
                    setData((d) => ({ ...d, timeMin: parseInt(e.target.value) || 0 }))
                  }
                />
                <span className="text-slate-400 font-bold">:</span>
                <input
                  ...
                  value={data.timeSec}
                  onChange={(e) =>
                    setData((d) => ({ ...d, timeSec: parseInt(e.target.value) || 0 }))
                  }
                />
              </div>
            </div>
          ...
          {data.type === 'SHOT' && (
            <div className="flex gap-3 mb-3 items-end flex-wrap">
                ...
                <div className="flex gap-1">
                  {[2, 3].map((v) => (
                    <button
                      key={v}
                      className={
                        'px-3 py-1.5 rounded text-xs font-bold cursor-pointer ' +
                        (data.val === v
              ...
              <div>
                ...
                <div className="flex gap-1">
                  <button
                    className={
                      'px-3 py-1.5 rounded text-xs font-bold cursor-pointer ' +
                      (data.made ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300')
                    }
                    onClick={() => setData((d) => ({ ...d, made: true }))}
                  >
                    Marqué
                  </button>
                  <button
                    className={
                      'px-3 py-1.5 rounded text-xs font-bold cursor-pointer ' +
                      (!data.made ? 'bg-red-600 text-white' : 'bg-slate-700 text-slate-300')
                    }
                    onClick={() => setData((d) => ({ ...d, made: false }))}
                  >
                    Raté
                  </button>
                </div>
              ...
              <button
                className="px-3 py-1.5 rounded text-xs font-bold cursor-pointer bg-slate-700 text-slate-300 hover:bg-slate-600 self-end"
                onClick={() => setShowZone(true)}
              >
          ...
          {data.type === 'SHOT' && data.made && (
            <div className="mb-3">
              ...
              <CombinedPlayerSelect
                value={data.astId}
                onChange={(v) =>
                  setData((d) => ({ ...d, astId: v, hockeyAssistId: v ? d.hockeyAssistId : null }))
                }
                homePlayers={players}
                oppPlayers={oppPlayers}
                allowNone={true}
            ...
            <div className="flex gap-3 mb-3 items-end">
              ...
              <div>
                ...
                <input
                  ...
                  value={data.ftMade}
                  onChange={(e) =>
                    setData((d) => ({ ...d, ftMade: parseInt(e.target.value) || 0 }))
                  }
                />
              </div>
            </div>
                ...
                <div className="flex gap-1 flex-wrap">
                  {FOUL_TYPES.map((ft) => (
                    <button
                      key={ft.value}
                      className={
                        'px-3 py-1.5 rounded text-xs font-bold cursor-pointer border transition-all ' +
                        (data.foulType === ft.value
                          ? ft.color + ' border-white text-white'
                      ...
                      onClick={() =>
                        setData((d) => ({
                          ...d,
                          foulType: ft.value,
                          victim: ft.value === 'TECHNICAL' ? null : d.victim,
                          ftShooterId: ft.value !== 'TECHNICAL' ? null : d.ftShooterId,
                        }))
                      }
                    >
                      {ft.label}
          ...
          {data.type === 'TOV' && (
            <div className="flex items-center gap-2 mb-3">
              <input
                type="checkbox"
                checked={data.unforced || false}
                onChange={(e) => setData((d) => ({ ...d, unforced: e.target.checked }))}
                style={{ accentColor: '#ef4444' }}
              />
              <span className="text-xs text-slate-400">
                Perte non-forcée (erreur de concentration)
              </span>
            </div>
          )}
          {data.type === 'STL' && (
          ...
          {data.type === 'STOPPAGE' && (
            <div className="mb-3">
              ...
              <input
                ...
                value={data.duration}
                onChange={(e) =>
                  setData((d) => ({ ...d, duration: parseInt(e.target.value) || 30 }))
                }
              />
            </div>
          )}
  ...
  function PlayByPlayEditor({ game, players, phases = [], onSave, onClose }) {
    ...
    const handlePanelSave = (data) => {
      if (editingAction._new) {
        ...
        if (data.type === 'SHOT') {
          newAction.val = data.val;
          newAction.made = data.made;
          ...
          if (data.made && data.astId) {
            ...
            if (data.hockeyAssistId)
              newAction.hockeyAssistId =
                data.hockeyAssistId === 'OPP' ? 'OPP' : parseInt(data.hockeyAssistId);
          }
          if (data.zoneX !== undefined) {
            newAction.x = data.zoneX;
            newAction.y = data.zoneY;
          }
        }
        if (data.type === 'FT') {
          newAction.ftMade = data.ftMade;
          newAction.ftAtt = data.ftAtt;
        }
        if (data.type === 'FOUL') {
          newAction.foulType = data.foulType;
          if (data.foulType === 'TECHNICAL') {
            if (data.ftShooterId)
              newAction.ftShooterId =
                data.ftShooterId === 'OPP' ? 'OPP' : parseInt(data.ftShooterId);
          } else {
        ...
        if (data.type === 'STOPPAGE') {
          newAction.duration = data.duration || 30;
          newAction.pid = 0;
        }
        if (data.type === 'TIMEOUT') {
          newAction.team = data.team;
          newAction.pid = 0;
        }
        ...
        setActions((prev) =>
          prev.map((a) => {
            ...
            delete u.subOut;
            delete u.astId;
            delete u.hockeyAssistId;
            delete u.ftShooterId;
            if (data.type === 'SHOT') {
              u.val = data.val;
              u.made = data.made;
              if (data.shotQuality) u.shotQuality = data.shotQuality;
              else delete u.shotQuality;
              if (data.made && data.astId) {
            ...
            if (data.type === 'FOUL') {
              ...
              if (data.foulType === 'TECHNICAL') {
                if (data.ftShooterId)
                  u.ftShooterId = data.ftShooterId === 'OPP' ? 'OPP' : parseInt(data.ftShooterId);
              } else {
            ...
            if (data.type === 'SUB') {
              ...
              u.pid    = data.subOut;  // subOut du formulaire = sortant → pid
              u.inId   = data.pid;     // pid du formulaire = entrant → inId
              u.subOut = data.subOut;  // rétrocompat lecture
            }
            if (data.type === 'STOPPAGE') {
              u.duration = data.duration || 0;
              u.pid = 0;
            }
            if (data.type === 'TIMEOUT') {
              u.team = data.team || 'home';
              u.pid = 0;
            }
            if (data.play) u.play = data.play;
            else delete u.play;
            return u;
          })
        );
