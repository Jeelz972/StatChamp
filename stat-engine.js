...
window.StatsEngine = {
  ...
  possAdvanced(team, opp) {
    ...
    const tPoss = team.fga + this.CONSTANTS.FTA_WEIGHT_ADV * team.fta
      - this.CONSTANTS.ORB_FACTOR * tORB_pct * (team.fga - team.fgm) + team.tov;
    const oPoss = opp.fga + this.CONSTANTS.FTA_WEIGHT_ADV * (opp.fta || 0)
      - this.CONSTANTS.ORB_FACTOR * oORB_pct * (opp.fga - (opp.fgm || 0)) + (opp.tov || 0);
  ...
  PIE(player, gameTotal) {
    const num = player.pts + player.fgm + player.ftm - player.fga - player.fta
      + player.dreb + (this.CONSTANTS.THREE_BONUS * player.oreb)
      + player.ast + player.stl + (this.CONSTANTS.THREE_BONUS * player.blk)
      - player.pf - player.tov;
    const den = gameTotal.pts + gameTotal.fgm + gameTotal.ftm - gameTotal.fga - gameTotal.fta
      + gameTotal.drb + (this.CONSTANTS.THREE_BONUS * gameTotal.orb)
      + gameTotal.ast + gameTotal.stl + (this.CONSTANTS.THREE_BONUS * gameTotal.blk)
      - gameTotal.pf - gameTotal.tov;
  ...
  dirtyWork(oreb, dreb, stl, blk, chargesTaken, deflections, minutes) {
    const raw = (oreb * 2) + (dreb * 0.5) + (stl * 1.5) + (blk * 1.5)
      + (chargesTaken * 3) + (deflections * 1);
    return minutes > 0 ? (raw / minutes) * this.CONSTANTS.PER_MIN : raw;
  },
  ...
  impactTotal(ois, dis, minutes) {
    return this.safe(ois + dis, minutes, this.CONSTANTS.IMPACT_MIN);
  },
   ...
   */
  calcORtg(params) {
    const { MP, PTS, FGM, FGA, ThreePM, FTM, FTA, ORB, AST, TOV,
      Team_PTS, Team_FGM, Team_FGA, Team_ThreePM, Team_FTM, Team_FTA,
      Team_ORB, Team_AST, Team_MP, Opp_DRB } = params;
    ...
    const FT_Scoring     = FTA > 0 ? (1 - Math.pow(1 - (FTM / FTA), 2)) * this.CONSTANTS.FTA_WEIGHT_ADV * FTA : 0;
    const Team_FT_Scoring = Team_FTA > 0
      ? (1 - Math.pow(1 - (Team_FTM / Team_FTA), 2)) * this.CONSTANTS.FTA_WEIGHT_ADV * Team_FTA : 0;
    const Team_Scoring_Poss  = Team_FGM + Team_FT_Scoring;
    const Team_ORB_Weight    = Team_Scoring_Poss > 0
      ? ((1 - Team_ORB_Pct) * Team_Play_Pct) / ((1 - Team_ORB_Pct) * Team_Play_Pct + Team_ORB_Pct * (1 - Team_Play_Pct))
      : 0;

    const qAST_t1 = (MP / (Team_MP / 5)) * (1.14 * ((Team_AST - AST) / (Team_FGM || 1)));
    const qAST_t2 = ((((Team_AST / Team_MP) * MP * 5 - AST) / ((Team_FGM / Team_MP) * MP * 5 - FGM || 1))
      * (1 - (MP / (Team_MP / 5))));
    const qAST = Math.min(Math.max(qAST_t1 + qAST_t2, 0), 1) || 0;

    const FG_Part  = FGM * (1 - this.CONSTANTS.THREE_BONUS * ((PTS - FTM) / (2 * FGA || 1)) * qAST);
    const AST_Part = this.CONSTANTS.THREE_BONUS
      * (((Team_PTS - Team_FTM) - (PTS - FTM)) / (2 * (Team_FGA - FGA) || 1)) * AST;
    const ScPoss_Factor = Team_Scoring_Poss > 0
      ? 1 - (Team_ORB / Team_Scoring_Poss) * Team_ORB_Weight * Team_Play_Pct : 1;
    ...
    const PProd_FG  = 2 * (FGM + this.CONSTANTS.THREE_BONUS * ThreePM)
      * (1 - this.CONSTANTS.THREE_BONUS * ((PTS - FTM) / (2 * FGA || 1)) * qAST);
    const PProd_AST = 2
      * ((Team_FGM - FGM + this.CONSTANTS.THREE_BONUS * (Team_ThreePM - ThreePM)) / (Team_FGM - FGM || 1))
  ...
  calcDRtg(params) {
    ...
    const FMwt           = (FMwt_D + (1 - DFG_Pct) * DOR_Pct) > 0
      ? FMwt_D / (FMwt_D + (1 - DFG_Pct) * DOR_Pct) : 0.5;

    const Stops1         = STL + BLK * FMwt * (1 - this.CONSTANTS.ORB_FACTOR * DOR_Pct) + DRB * (1 - FMwt);
    const Stops2_P1      = Team_MP > 0
      ? ((Opp_FGA - Opp_FGM - Team_BLK) / Team_MP) * FMwt * (1 - this.CONSTANTS.ORB_FACTOR * DOR_Pct) : 0;
    const Stops2_P2      = Team_MP > 0 ? ((Opp_TOV - Team_STL) / Team_MP) : 0;
    const Stops2_P3      = Team_PF > 0 && Opp_FTA > 0
      ? (PF / Team_PF) * this.CONSTANTS.FTA_WEIGHT_ADV * Opp_FTA * Math.pow(1 - (Opp_FTM / Opp_FTA), 2) : 0;
    const Stops           = Stops1 + (Stops2_P1 + Stops2_P2) * MP + Stops2_P3;

    const Stop_Pct       = (Team_Poss * MP) > 0
      ? (Stops * (Opp_MP || Team_MP)) / (Team_Poss * MP) : 0;
    const Team_DRtg      = Team_Poss > 0 ? 100 * (Opp_PTS / Team_Poss) : 100;
    const Opp_FT_Scoring = Opp_FTA > 0
      ? (1 - Math.pow(1 - (Opp_FTM / Opp_FTA), 2)) * this.CONSTANTS.FTA_WEIGHT_ADV * Opp_FTA : 0;
    const D_Pts_per_ScPoss = (Opp_FGM + Opp_FT_Scoring) > 0
      ? Opp_PTS / (Opp_FGM + Opp_FT_Scoring) : 2;
   ...
   */
  fourFactors(s) {
    const eFG     = s.fga > 0 ? parseFloat(((s.fgm + this.CONSTANTS.THREE_BONUS * s.threePM) / s.fga * 100).toFixed(1)) : 0;
    const tovPct  = (s.fga + this.CONSTANTS.FTA_WEIGHT * s.fta + s.tov) > 0
      ? parseFloat((s.tov / (s.fga + this.CONSTANTS.FTA_WEIGHT * s.fta + s.tov) * 100).toFixed(1)) : 0;
  ...
  fatigueProfile(qStats) {
    ...
    let breakpoint = null, maxDrop = 0;
    for (let i = 1; i < n; i++) {
      const drop = effs[i - 1] - effs[i];
      if (drop > maxDrop && drop > 0.3) { maxDrop = drop; breakpoint = sorted[i].q; }
    }
    return {
    ...
    for (let iter = 0; iter < maxIter; iter++) {
      ...
      centroids = Array.from({ length: k }, (_, ci) => {
        ...
        return Array.from({ length: dims }, (_, dim) =>
          members.reduce((s, p) => s + p[dim], 0) / members.length
        );
      });
    }
    return assignments;
