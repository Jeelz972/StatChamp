        ...
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 0 }}>
            ...
            <ReferenceLine
              ...
              stroke="#3a3a5a"
              strokeDasharray="3 3"
              label={{ value: 'Médiane Vol.', fill: '#3a3a5a', fontSize: 8, position: 'top' }}
            />
            <ReferenceLine
              y={data.medTs}
              stroke="#3a3a5a"
              strokeDasharray="3 3"
              label={{ value: 'Médiane Eff.', fill: '#3a3a5a', fontSize: 8, position: 'right' }}
            />
            <Tooltip
              ...
              <tbody className="divide-y divide-slate-800/50">
                {sorted.map((p) => (
                  ...
                  >
                    ...
                    <td
                      className={`p-3 text-center ${parseFloat(p.avg.fgPct) >= 45 ? 'text-green-400' : parseFloat(p.avg.fgPct) < 35 ? 'text-red-400' : 'text-slate-300'}`}
                    >
                      {p.avg.fgPct}%
                    </td>
                    <td
                      className={`p-3 text-center ${parseFloat(p.avg.threePct) >= 33 ? 'text-blue-400' : 'text-slate-500'}`}
                    >
                      {p.avg.threePct}%
                    </td>
      ...
      {selectedPlayer && (
        ...
        >
          <div className="space-y-6">
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700 mb-6">
              ...
              <div className="grid grid-cols-4 md:grid-cols-8 gap-3 text-center">
                <div>
                  ...
                  <div className="text-sm font-bold text-white">{selectedPlayer.avg.twoPct}%</div>
                ...
                <div>
                  ...
                  <div className="text-sm font-bold text-white">{selectedPlayer.avg.ftPct}%</div>
                  <div className="text-[10px] text-slate-600">
            ...
            {/* --- BLOC SPLIT W/L --- */}
            {selectedPlayer.splitWL &&
              (selectedPlayer.splitWL.win || selectedPlayer.splitWL.loss) && (
                <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700 mb-6">
                  <h4 className="text-xs text-slate-500 uppercase font-bold mb-3">
                    Performance <span className="text-green-400">Victoires</span> vs{' '}
                      ...
                      <tbody className="divide-y divide-slate-800">
                        {selectedPlayer.splitWL.win && (
                          <tr>
                            ...
                            <td className="p-2 text-center text-slate-400">
                              {selectedPlayer.splitWL.win.gp}
                            </td>
                            <td className="p-2 text-center text-slate-500">
                              {selectedPlayer.splitWL.win.min}
                            </td>
                            <td className="p-2 text-center font-bold text-white">
                              {selectedPlayer.splitWL.win.pts}
                            </td>
                            ...
                            <td className="p-2 text-center text-red-400">
                              {selectedPlayer.splitWL.win.tov}
                            </td>
                            <td className="p-2 text-center">{selectedPlayer.splitWL.win.fgPct}%</td>
                            <td className="p-2 text-center">
                              {selectedPlayer.splitWL.win.threePct}%
                            </td>
                            <td
                              className={`p-2 text-center font-bold ${parseFloat(selectedPlayer.splitWL.win.plusMinus) >= 0 ? 'text-green-400' : 'text-red-400'}`}
                            >
                              {parseFloat(selectedPlayer.splitWL.win.plusMinus) > 0 ? '+' : ''}
                              {selectedPlayer.splitWL.win.plusMinus}
                            </td>
                            <td className="p-2 text-center font-bold text-green-400">
                              {selectedPlayer.splitWL.win.eff}
                            </td>
                          </tr>
                        )}
                        {selectedPlayer.splitWL.loss && (
                          <tr>
                            ...
                            <td className="p-2 text-center text-slate-400">
                              {selectedPlayer.splitWL.loss.gp}
                            </td>
                            <td className="p-2 text-center text-slate-500">
                              {selectedPlayer.splitWL.loss.min}
                            </td>
                            <td className="p-2 text-center font-bold text-white">
                              {selectedPlayer.splitWL.loss.pts}
                            </td>
                            ...
                            <td className="p-2 text-center text-red-400">
                              {selectedPlayer.splitWL.loss.tov}
                            </td>
                            <td className="p-2 text-center">
                              {selectedPlayer.splitWL.loss.fgPct}%
                            </td>
                            <td className="p-2 text-center">
                              {selectedPlayer.splitWL.loss.threePct}%
                            </td>
                            <td
                              className={`p-2 text-center font-bold ${parseFloat(selectedPlayer.splitWL.loss.plusMinus) >= 0 ? 'text-green-400' : 'text-red-400'}`}
                            >
                              {parseFloat(selectedPlayer.splitWL.loss.plusMinus) > 0 ? '+' : ''}
                              {selectedPlayer.splitWL.loss.plusMinus}
                            </td>
                            <td className="p-2 text-center font-bold text-green-400">
                              {selectedPlayer.splitWL.loss.eff}
                            </td>
                          </tr>
                        )}
                      </tbody>
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-2 bg-slate-900 p-4 rounded-lg">
              <div className="text-center">
                ...
                <div className="text-xl md:text-2xl font-bold text-white">
                  {selectedPlayer.avg.pts}
                </div>
              </div>
              <div className="text-center">
                <div className="text-xs text-slate-500">Rebonds</div>
                <div className="text-xl md:text-2xl font-bold text-white">
                  {selectedPlayer.avg.reb}
                </div>
              </div>
              <div className="text-center">
                <div className="text-xs text-slate-500">Passes</div>
                <div className="text-xl md:text-2xl font-bold text-white">
                  {selectedPlayer.avg.ast}
                </div>
              </div>
              <div className="text-center">
                <div className="text-xs text-slate-500">Éval</div>
                <div className="text-xl md:text-2xl font-bold text-green-400">
                  {selectedPlayer.avg.eff}
                </div>
              </div>
              <div className="text-center">
                <div className="text-xs text-slate-500">PIR</div>
                <div className="text-xl md:text-2xl font-bold text-orange-400">
                  {selectedPlayer.avg.pir}
                </div>
              </div>
              <div className="text-center">
                <div className="text-xs text-slate-500">PIE</div>
                <div className="text-xl md:text-2xl font-bold text-cyan-400">
                  {selectedPlayer.avg.PIE}%
                </div>
              </div>
              ...
              <div className="text-center">
                ...
                <div
                  className={`text-2xl font-black ${parseFloat(selectedPlayer.avg.plusMinus) >= 0 ? 'text-green-400' : 'text-red-400'}`}
                >
                  {parseFloat(selectedPlayer.avg.plusMinus) > 0 ? '+' : ''}
                  {selectedPlayer.avg.plusMinus}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div
                    key={item.key}
                    className={`relative overflow-hidden rounded-xl bg-gradient-to-br ${item.color} p-0.5`}
                  >
                    <div className="bg-slate-900 rounded-xl p-3 h-full">
                      ...
                      <div className="text-xl font-black text-white">
                        {selectedPlayer.records[item.key]}
                      </div>
                      <div className="text-[9px] text-slate-500 mt-1 truncate">
                        vs {selectedPlayer.records[item.key + 'Opp']}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-4 gap-2">
              ...
              <div className="bg-slate-800 p-3 rounded-lg text-center border border-slate-700">
                ...
                <div
                  className={`text-lg font-bold ${parseFloat(selectedPlayer.avg.netRtg) >= 0 ? 'text-green-400' : 'text-red-400'}`}
                >
                  {parseFloat(selectedPlayer.avg.netRtg) > 0 ? '+' : ''}
                  {selectedPlayer.avg.netRtg}
                </div>
              </div>
              <div className="bg-slate-800 p-3 rounded-lg text-center border border-slate-700">
            ...
            <div className="bg-slate-900 p-4 rounded-lg border border-slate-700">
              <h4 className="text-xs text-slate-400 uppercase mb-3 font-bold tracking-wider">
                Adresses
              </h4>
              <div className="grid grid-cols-3 gap-4">
                {[
                  {
                    label: '2 Points',
                    pct: selectedPlayer.avg.twoPct,
                    made: selectedPlayer.stats.fgm,
                    att: selectedPlayer.stats.fga,
                    color: 'orange',
                  },
                  {
                    label: '3 Points',
                    pct: selectedPlayer.avg.threePct,
                    made: selectedPlayer.stats.threePM,
                    att: selectedPlayer.stats.threePA,
                    color: 'blue',
                  },
                  {
                    label: 'Lancers F.',
                    pct: selectedPlayer.avg.ftPct,
                    made: selectedPlayer.stats.ftm,
                    att: selectedPlayer.stats.fta,
                    color: 'green',
                  },
                ].map((sh) => (
                  <div key={sh.label} className="text-center">
                    ...
                    <div className="w-full bg-slate-700 rounded-full h-2 mt-2 overflow-hidden">
                      <div
                        className={`h-full rounded-full bg-${sh.color}-500 transition-all`}
            ...
            {(selectedPlayer.logs || []).length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-900 rounded-lg p-3 border border-slate-700">
                  <h4 className="text-xs text-slate-400 uppercase mb-2 font-bold">
                    Scoring & Éval par match
                  </h4>
                ...
                <div className="overflow-x-auto bg-slate-900 rounded-lg border border-slate-700 max-h-64">
                  <table className="w-full text-xs text-slate-300 whitespace-nowrap">
                    <thead className="bg-slate-800 text-white uppercase text-[10px] sticky top-0 z-10">
                      <tr>
                        <th className="p-2 text-left sticky left-0 bg-slate-800 z-20">
                          Adversaire
                        </th>
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {(selectedPlayer.logs || []).map((log, i) => (
                        <tr
                          key={i}
                          className={`transition-colors ${log.isWin ? 'bg-green-900/5 hover:bg-green-900/15' : 'bg-red-900/5 hover:bg-red-900/15'}`}
                        >
                          <td className="p-2 font-bold text-white sticky left-0 bg-slate-900 z-10 border-r border-slate-800">
                            <span
                              className={`inline-block w-1.5 h-1.5 rounded-full mr-1.5 ${log.isWin ? 'bg-green-500' : 'bg-red-500'}`}
                            ></span>
                            {log.opponent}
                          </td>
                          <td className="p-2 text-center">
                          ...
                          <td
                            className={`p-2 text-center font-bold ${log.plusMinus >= 0 ? 'text-green-400' : 'text-red-400'}`}
                          >
                            {log.plusMinus > 0 ? '+' : ''}
                            {log.plusMinus}
                          </td>
