...
export default function Home() {
  ...
  const seasonGames = useMemo(() => gamesForActiveSeason(), [gamesForActiveSeason, games, phases, activeSeason]);

  const seasonStats = useMemo(() => {
    const n = seasonGames.length;
    if (!n) return { wins: 0, losses: 0, ptsFor: '—', ptsAgainst: '—' };
    const wins = seasonGames.filter((g) => (g.homeScore ?? 0) > (g.awayScore ?? 0)).length;
    const ptsFor = (seasonGames.reduce((acc, g) => acc + (g.homeScore ?? 0), 0) / n).toFixed(1);
    const ptsAgainst = (seasonGames.reduce((acc, g) => acc + (g.awayScore ?? 0), 0) / n).toFixed(1);
    return { wins, losses: n - wins, ptsFor, ptsAgainst };
  }, [seasonGames]);
    ...
    <div style={{ maxWidth: '56rem', margin: '0 auto', padding: '1.5rem' }}>
          ...
          {lastGame ? (
            <div
              ...
              style={{
                ...
                padding: '0.75rem', borderRadius: '0.5rem', background: 'var(--bg-3)',
            ...
            >
              ...
              <div style={{ fontSize: '1.625rem', fontWeight: 900, color: 'var(--text-1)', fontVariantNumeric: 'tabular-nums' }}>
      ...
      <div className="sc-card" style={{ marginBottom: '1.25rem' }}>
        <div className="sc-card-header">
          Bilan saison
          {activeSeason && (
            <span style={{ marginLeft: '0.5rem', color: 'var(--accent)', textTransform: 'none', fontWeight: 600 }}>
        ...
        <div style={{ padding: '1rem' }}>
          {!activeSeason ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem' }}>
              ...
              <StatBlock label="PTS enc. / match" value={seasonStats.ptsAgainst} color="#94a3b8" />
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{ padding: '1rem' }}>
          {!activeSeason ? (
            ...
            <div style={{ textAlign: 'center', padding: '1rem', color: 'var(--text-3)', fontSize: '0.875rem' }}>
              ...
              <button onClick={() => setView('settings')} style={{ color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
                Gérer l'effectif
              </button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
              {sortedRoster.map((player) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-3)', width: '2.25rem', textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
                      #{player.number}
                    </span>
                    <span style={{ flex: 1, fontWeight: 600, color: 'var(--text-1)', fontSize: '0.875rem' }}>
                      {player.name}
                    </span>
                    <span style={{ width: '0.5rem', height: '0.5rem', borderRadius: '50%', background: cfg.color, flexShrink: 0 }} />
                    {isAdmin ? (
                      <select
                        ...
                        style={{
                          ...
                          color: 'var(--text-1)', borderRadius: '0.25rem',
                          padding: '0.125rem 0.25rem', fontSize: '0.75rem',
...
function StatBlock({ label, value, color }) {
  return (
    <div style={{
      background: 'var(--bg-3)', borderRadius: '0.5rem', padding: '0.875rem 1rem',
      textAlign: 'center', border: '1px solid var(--border)',
    }}>
      <div style={{ fontSize: '1.75rem', fontWeight: 900, color, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
        {value}
      </div>
      <div style={{ fontSize: '0.6875rem', color: 'var(--text-3)', marginTop: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 }}>
        {label}
      </div>
    </div>
  );
