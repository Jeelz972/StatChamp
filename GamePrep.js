...
function GamePrep({ opponentName, games, players, onBack }) {
  ...
  const saveToFirestore = async () => {
    ...
    setSaving(true);
    try {
      await window.db.collection('gamePreps').doc(prepId).set(
        {
        ...
        <window.Card className="p-4">
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-center">
            ...
            {[
              ...
              <div key={s.label} className="bg-slate-900/50 rounded p-2 border border-slate-700/30">
