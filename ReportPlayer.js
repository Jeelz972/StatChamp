...
const AnalysisEngine = {
    ...
    // Trouver le meilleur match NBA
    findNBAComparison: function (fingerprint, shotProfile) {
        var self = this;
        ...
        // Initialisation à -1 pour s'assurer que les premiers passages écrasent ces valeurs
        var best = { name: null, similarity: -1, desc: '' };
        var second = { name: null, similarity: -1, desc: '' };
        ...
        var maxEuc = 100 * Math.sqrt(dims.length); // ~282.8

        profiles.forEach(function (nba) {
            // 1. Cosine similarity (Forme du profil / Style de jeu)
            var cosSim = self._cosineSim(fingerprint, nba.fp);

            // 2. Distance euclidienne normalisée (Niveau global / Position absolue)
            var eucDist = 0;
            dims.forEach(function (d) {
                var diff = (fingerprint[d] || 0) - (nba.fp[d] || 0);
                eucDist += diff * diff;
            });
            var eucSim = 1 - Math.sqrt(eucDist) / maxEuc;

            // 3. Hybride : 60% position + 40% forme
            // (La similarité cosinus "tasse" les résultats vers le haut, on compense avec l'Euclidien)
            var fpSim = cosSim * 0.7 + eucSim * 0.3;

            // 4. Bonus shot profile
            var hasShotData = shotProfile && shotProfile.totalShots >= 10 && nba.shot;
            var shotSim = hasShotData ? self._shotSim(shotProfile, nba.shot) : 0;

            var totalSimRaw = hasShotData ? fpSim * 0.8 + shotSim * 0.2 : fpSim;

            // Conversion IMMÉDIATE en score sur 100 pour éviter le bug de comparaison
            var finalScore = Math.round(totalSimRaw * 100);

            // 5. Mise à jour du classement
            if (finalScore > best.similarity) {
                // Le 1er devient 2ème
                second = { name: best.name, similarity: best.similarity, desc: best.desc };
                // On enregistre le nouveau 1er
                best = { name: nba.name, similarity: finalScore, desc: nba.desc };
            } else if (finalScore > second.similarity) {
                // On met à jour seulement le 2ème
                second = { name: nba.name, similarity: finalScore, desc: nba.desc };
            }
        });

        // Si le meilleur score est toujours -1 (erreur inattendue), on remet à 0
        if (best.similarity === -1) best.similarity = 0;
        if (second.similarity === -1) second.similarity = 0;
    ...
    getArchetype: function (p, allPlayers, games) {
        ...
        var nbaComp = AnalysisEngine.findNBAComparison(fp, sp);
    ...
    _classifyFromFingerprint: function (fp, avg, gp, nbaComp, B) {
        ...
        var tags = [];
        if (fp.shooting >= 55 && threeaPerGame >= 2) tags.push('Tire');
        if (fp.creation >= 55 && (avg.ast || 0) >= 2) tags.push('Crée');
        if (fp.defense >= 55) tags.push('Défend');
        if (fp.rebounding >= 55) tags.push('Rebond');
        if (fp.efficiency >= 65 && fp.volume >= 40) tags.push('Finit');
        if (fp.interior >= 60 && (avg.oreb || 0) >= B.oreb_good * 0.7) tags.push('Hustle');
        if ((avg.FTr || 0) >= 0.3) tags.push('Provoque');
        if (fp.impact >= 65) tags.push('Impact+');
        tags = tags.slice(0, 3);
        ...
        var secondary = s2 >= 40 ? DIM_LABELS[top2] : null;
        ...
        var ARCHETYPE_MATRIX = {
            ...
            'rebounding+interior': {
                ...
                border: 'border-blue-500',
                bg: 'bg-blue-900/20',
                desc: 'Contrôle la peinture des deux côtés.',
            },
            'rebounding+impact': {
                name: 'Guerrier',
                tier: 3,
            ...
            'defense+interior': {
                ...
                border: 'border-blue-600',
                bg: 'bg-blue-900/20',
                desc: 'Ancre défensive intérieure.',
            },
            'defense+rebounding': {
                name: 'Ancre Intérieure',
                tier: 2,
        ...
        var key1 = top1 + '+' + top2;
        var key2 = top2 + '+' + top1;
        var match = ARCHETYPE_MATRIX[key1] || ARCHETYPE_MATRIX[key2];

        // Override Croqueur
        if (top1 === 'volume' && fp.volume >= 70 && fp.efficiency < 35) {
            match = {
                name: 'Croqueur',
                tier: 1,
                color: 'text-red-400',
        ...
        // Override Spot-Up
        if (!match && fp.shooting >= 45 && threeaPerGame >= 2) {
            match = {
                name: 'Spot-Up',
                tier: 3,
                color: 'text-sky-400',
                ...
                {arch.nbaComp && arch.nbaComp.best && arch.nbaComp.best.name && (
                    <div className="bg-slate-950/60 border border-slate-800/50 rounded-lg p-3">
                        <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">
                            Profil comparable NBA
                        </div>
                        <div className="flex items-center gap-3">
                            <span className="text-lg font-bold text-orange-400">{arch.nbaComp.best.name}</span>
                            <span className="text-xs px-2 py-0.5 rounded-full bg-orange-900/30 text-orange-300 border border-orange-800">
                                {arch.nbaComp.best.similarity}% match
                            </span>
                        </div>
                        <p className="text-[11px] text-slate-400 mt-1">{arch.nbaComp.best.desc}</p>
                        {arch.nbaComp.second && arch.nbaComp.second.name && (
                            <div className="text-[10px] text-slate-600 mt-1.5">
                                Aussi comparable à{' '}
                                <span className="text-slate-400">{arch.nbaComp.second.name}</span>
                                <span className="text-slate-600 ml-1">({arch.nbaComp.second.similarity}%)</span>
                            </div>
                        )}
                    </div>
                )}
