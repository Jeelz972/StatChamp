...
const parseDate = (dateStr) => {
    ...
    if (match) { const m = months[match[2].toLowerCase().replace('.', '')]; if (m !== undefined) return new Date(match[3], m, match[1]); }
...
const Icons = {
    ...
    Link: "M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71", Cloud: "M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z",
...
// MODAL RESPONSIVE AVEC Z-INDEX ÉLEVÉ
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 no-print">
            <div className={`bg-slate-800 rounded-xl border border-slate-600 w-full ${size} shadow-2xl max-h-[90vh] flex flex-col`}>
                <div className="flex justify-between items-center p-3 md:p-4 border-b border-slate-700 shrink-0">
...
function LoginModal({ isOpen, onLogin, onClose }) {
    ...
    if (!isOpen) return null;
    const handleLogin = () => { if (pwd === ADMIN_PASSWORD) { onLogin(); onClose(); } else { setError(true); } };
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Accès Coach" size="max-w-sm">
            <div className="space-y-4 p-2"><p className="text-sm text-slate-400">Code d'accès requis.</p><input type="password" className="w-full bg-slate-900 text-white p-3 rounded border border-slate-700" placeholder="Mot de passe..." value={pwd} onChange={(e) => { setPwd(e.target.value); setError(false); }} onKeyPress={(e) => e.key === 'Enter' && handleLogin()} />{error && <div className="text-red-500 text-xs">Incorrect</div>}<div className="flex justify-end gap-2"><Button variant="ghost" onClick={onClose}>Annuler</Button><Button variant="primary" onClick={handleLogin}>OK</Button></div></div>
        </Modal>
    );
}
...
// --- MODALE DÉTAILS MATCH (Responsive + Stats) ---
function GameDetailsModal({ game, isOpen, onClose, players }) {
    ...
    const statsData = useMemo(() => {
        ...
        Object.values(pStats).forEach(s => { T_FGM+=(s.fgm||0)+(s.threePM||0); T_FGA+=(s.fga||0)+(s.threePA||0); T_3PM+=(s.threePM||0); T_FTM+=(s.ftm||0); T_FTA+=(s.fta||0); T_ORB+=(s.oreb||0); T_DRB+=(s.dreb||0); T_AST+=(s.ast||0); T_STL+=(s.stl||0); T_BLK+=(s.blk||0); T_TOV+=(s.tov||0); T_PF+=(s.pf||0); T_PTS+=(s.pts||0); T_MP+=(s.minutes||0); });
        
        const O_PTS = game.awayScore||0; const O_FGM=opp.fgm||0; const O_FGA=opp.fga||(O_FGM+T_DRB); const O_FTM=opp.ftm||0; const O_FTA=opp.fta||0; const O_ORB=opp.oreb||0; const O_DRB=(opp.reb||(O_ORB+T_DRB))-O_ORB; const O_TOV=opp.tov||0;
        ...
        const rawPlayers = Object.entries(pStats).map(([pid, s]) => {
            ...
            return { id: pid, name: player?player.name:`#${pid}`, minutes: MP, pts: s.pts||0, ast: s.ast||0, reb: (s.oreb||0)+(s.dreb||0), stl: s.stl||0, blk: s.blk||0, tov: s.tov||0, pf: s.pf||0, fgm: FGM, fga: FGA, threePM: s.threePM||0, threePA: s.threePA||0, ftm: s.ftm||0, fta: s.fta||0, oreb: s.oreb||0, dreb: s.dreb||0, plusMinus: s.plusMinus||0, eFG: eFG.toFixed(1), TS: TS.toFixed(1) };
        }).filter(p => p!==null);
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`Détails: vs ${game.opponent}`} size="max-w-6xl">
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-2 bg-slate-900 p-3 md:p-4 rounded-lg flex justify-between items-center border border-slate-700 relative overflow-hidden">
                        <div className="text-center z-10"><div className="text-xs text-slate-400 uppercase">Nous</div><div className="text-3xl md:text-4xl font-bold text-green-400">{game.homeScore}</div></div>
                        <div className="flex flex-col items-center z-10"><span className="text-lg md:text-xl font-bold text-white uppercase tracking-wider truncate max-w-[120px] md:max-w-none">{game.opponent}</span><span className="text-xs text-slate-500 mt-1">{game.date}</span></div>
                        <div className="text-center z-10"><div className="text-xs text-slate-400 uppercase">Eux</div><div className="text-3xl md:text-4xl font-bold text-red-400">{game.awayScore}</div></div>
                    ...
                    <div className="bg-slate-800 p-3 md:p-4 rounded-lg border border-slate-600 flex flex-col justify-center">
                        <div className="text-xs text-slate-400 uppercase mb-2 text-center border-b border-slate-700 pb-1">Performance (Est.)</div>
...
function LiveTracker({ players, onSaveGame, initialGame, phases, selectedPhase }) {
    ...
    return (
        <div className="flex flex-col h-full gap-3 md:gap-4 relative pb-20 md:pb-0">
            <div className="bg-slate-800 p-2 flex justify-between items-center rounded-lg border border-slate-700">
                <div className="flex items-center gap-2 md:gap-4"><Button size="sm" className={isTimerRunning?"bg-red-500":"bg-green-500"} onClick={()=>setIsTimerRunning(!isTimerRunning)}><Icon path={isTimerRunning?Icons.Pause:Icons.Play}/> <span className="hidden md:inline">{isTimerRunning?"Stop":"Start"}</span></Button><div className="font-mono text-lg md:text-xl text-white font-bold">{Math.floor(gameTime/60).toString().padStart(2,'0')}:{(gameTime%60).toString().padStart(2,'0')}</div></div>
                <div className="flex items-center gap-2 md:gap-4"><select value={gameState.phase} onChange={(e)=>setGameState(p=>({...p,phase:e.target.value}))} className="bg-slate-700 text-white text-xs px-2 py-1 rounded border border-slate-600 max-w-[100px] md:max-w-none">{phases.map(ph=><option key={ph.id} value={ph.id}>{ph.name}</option>)}</select><span className="text-xs text-slate-400"><span className="hidden md:inline">Terrain: </span><span className={onCourt.length===5?"text-green-400":"text-orange-400"}>{onCourt.length}/5</span></span></div>
            </div>
            <Card className="bg-slate-900 p-2 md:p-4 flex justify-between items-center sticky top-0 z-10 border-b-4 border-coach-accent">
                <div className="text-3xl md:text-4xl font-bold text-white">{derived.homeScore}</div>
                <div className="flex flex-col items-center"><div className="text-lg md:text-xl font-bold text-orange-500">Q{gameState.quarter}</div><div className="flex gap-2 mt-1"><button onClick={()=>setGameState(p=>({...p,quarter:Math.max(1,p.quarter-1)}))} className="text-xs text-slate-500 hover:text-white px-2 bg-slate-800 rounded">-</button><button onClick={()=>setGameState(p=>({...p,quarter:p.quarter+1}))} className="text-xs text-slate-500 hover:text-white px-2 bg-slate-800 rounded">+</button></div></div>
                <div className="text-3xl md:text-4xl font-bold text-red-500">{derived.awayScore}</div>
            </Card>
            <div className="flex-1 overflow-y-auto grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                {players.map(player => {
                    const pStats = derived.playerStats[player.id] || { pts: 0, plusMinus: 0 };
                    const isOnCourt = onCourt.includes(player.id);
                    return (
                        <div key={player.id} className={`relative p-2 md:p-3 rounded-xl border shadow-md transition-all ${isOnCourt?'bg-slate-800 border-orange-500 ring-1 ring-orange-500/50':'bg-slate-800/60 border-slate-700 opacity-80'}`}>
                            ...
                            <div onClick={()=>setModal({type:"ACTION_MENU",data:player})} className="cursor-pointer">
                                <div className="flex justify-between pr-6"><span className={`font-bold truncate text-sm md:text-base ${isOnCourt?'text-white':'text-slate-400'}`}>{player.name}</span><span className="text-xs text-slate-500">#{player.number}</span></div>
            ...
            <div className="fixed bottom-16 md:bottom-0 left-0 right-0 bg-slate-900 p-3 md:p-4 border-t border-slate-800 flex justify-end z-20 gap-2"><Button variant="secondary" onClick={()=>setModal({type:"STATS"})}><Icon path={Icons.Eye}/> Stats</Button><Button variant="success" onClick={finalizeGame}><Icon path={Icons.Check}/> Finir</Button></div>
...
// --- GLOBAL STATS (Complet : Responsive + Dean Oliver + Sécurité) ---
function GlobalStats({ players, games, phases }) {
    ...
    const aggregated = useMemo(() => {
        ...
        filteredGames.forEach(g => {
            ...
            const teamPoss = (gameFGA-oppFGA) + 0.44*(gameFTA-oppFTA) - (gameORB-oppORB) + (gameTO-oppTOV); // Approx for daily ratings
            const teamORtg = teamPoss>0 ? ((gamePTS-oppPTS)/teamPoss)*100 : 100; // Simplified daily rating for log
            ...
            Object.entries(g.playerStats).forEach(([pid, s]) => {
                ...
                if ((s.minutes||0) > 0 && stats[id]) {
                    ...
                    stats[id].logs.push({ 
                        ...
                        // Note: Daily ratings in logs are approximations, full calculation is for Season Aggregates
                        ORtg: "0", DRtg: "0" 
                    });
                }
            });
        ...
        return activePlayers.map(p => {
            ...
            // Fill Logs with rough estimates for graphs if needed (optional)
            p.logs.forEach(l => { l.ORtg = ORtg.toFixed(1); l.DRtg = DRtg.toFixed(1); }); // Propagate Season Rating for context in graphs
...
// --- IMPORT MULTI-MATCHS (RESPONSIVE) ---
        ...
        <div className="space-y-4">
            ...
            <div className="overflow-y-auto max-h-[40vh] space-y-2">
                {importData.rawPlayers.map((imp, idx) => (
                    <div key={idx} className="flex flex-col md:flex-row items-start md:items-center gap-2 bg-slate-800 p-2 rounded border border-slate-700">
                        <div className="w-full md:w-1/3 flex justify-between md:block">
...
function Settings({ players, onUpdatePlayers, phases, onUpdatePhases, firebaseConfig, setFirebaseConfig }) {
    ...
    return (
        <div className="space-y-6 pb-20 md:pb-0">
            <Card className="p-4 md:p-6 border-l-4 border-orange-500">
                ...
                <div className="space-y-3">
                    {phases.map((ph, i) => (
                        <div key={ph.id} className="flex items-center gap-3 bg-slate-900 p-3 rounded border border-slate-700">
                            <div className="w-8 h-8 bg-orange-600 rounded-full flex items-center justify-center font-bold text-sm shrink-0">{i + 1}</div>
                            <input className="flex-1 bg-transparent text-white font-semibold outline-none min-w-0" value={ph.name} onChange={(e) => { const np = [...phases]; np[i].name = e.target.value; onUpdatePhases(np); }} />
                            <button onClick={() => { if (phases.length > 1 && confirm("Supprimer ?")) onUpdatePhases(phases.filter(p => p.id !== ph.id)); }} className="text-red-500 p-2 shrink-0"><Icon path={Icons.Trash} /></button>
                        </div>
                    ))}
                    <div className="flex gap-2 mt-4">
                        <input type="text" value={newPhaseName} onChange={(e) => setNewPhaseName(e.target.value)} placeholder="Nouvelle phase..." className="flex-1 bg-slate-900 text-white px-4 py-2 rounded border border-slate-600 min-w-0" onKeyPress={(e) => { if (e.key === 'Enter' && newPhaseName.trim()) { onUpdatePhases([...phases, { id: `phase_${generateId()}`, name: newPhaseName.trim() }]); setNewPhaseName(""); } }} />
            ...
            <div className="flex justify-between items-center"><h2 className="text-xl md:text-2xl font-bold text-white">Effectif</h2><Button onClick={() => onUpdatePlayers([...players, { id: Math.max(0, ...players.map(p => p.id)) + 1, name: "Nouveau", number: 0, pos: "G" }])} variant="success"><Icon path={Icons.Plus} /></Button></div>
            <div className="grid gap-3">
                {players.map((p, i) => (
                    <Card key={p.id} className="p-3 flex gap-3 items-center">
                        <input className="w-10 h-10 md:w-12 md:h-12 bg-slate-700 rounded-full text-center font-bold outline-none shrink-0" value={p.number} onChange={(e) => { const np = [...players]; np[i].number = parseInt(e.target.value) || 0; onUpdatePlayers(np); }} />
                        <input className="flex-1 bg-transparent text-white font-bold outline-none min-w-0" value={p.name} onChange={(e) => { const np = [...players]; np[i].name = e.target.value; onUpdatePlayers(np); }} />
                        <input className="w-12 md:w-16 bg-transparent text-slate-400 text-sm outline-none shrink-0 text-right" value={p.pos} onChange={(e) => { const np = [...players]; np[i].pos = e.target.value; onUpdatePlayers(np); }} />
                        <button onClick={() => onUpdatePlayers(players.filter(x => x.id !== p.id))} className="text-red-500 p-2 shrink-0"><Icon path={Icons.Trash} /></button>
                    </Card>
                ))}
            </div>
            
            <Card className="p-4 md:p-6 border-l-4 border-purple-500">
...
function App() {
    ...
    return (
        <div className="max-w-5xl mx-auto h-screen bg-slate-950 flex flex-col md:flex-row overflow-hidden font-sans text-slate-200">
            ...
            {/* NAVIGATION RESPONSIVE (SIDEBAR PC / BOTTOM BAR MOBILE) */}
            <nav className="
                fixed bottom-0 left-0 right-0 h-16 bg-slate-900 border-t border-slate-800 z-50 flex items-center justify-around
                md:relative md:h-full md:w-20 md:flex-col md:justify-start md:pt-6 md:border-t-0 md:border-r
            ">
                <div className="hidden md:block mb-4 p-2 bg-orange-600 rounded-xl text-white font-black text-xl cursor-default">BP</div>
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative">
                <header className="h-14 md:h-16 bg-slate-900 border-b border-slate-800 flex items-center px-4 md:px-6 shrink-0 z-20 justify-between">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        {!isAdmin && <span className="text-[10px] md:text-xs text-slate-500 px-2 py-1 bg-slate-800 rounded border border-slate-700">Public</span>}
                        {isAdmin && <span className="text-[10px] md:text-xs text-orange-500 px-2 py-1 bg-orange-900/20 rounded border border-orange-900">Admin</span>}
                        {window.db && <span className="text-[10px] md:text-xs text-green-400 flex items-center gap-1"><Icon path={Icons.Cloud} className="w-3 h-3"/> <span className="hidden sm:inline">Synchro</span></span>}
                    </div>
                </header>

                <div className="flex-1 p-2 md:p-4 overflow-y-auto z-10 scroll-pb-20">
                    {view === 'live' && isAdmin && (
