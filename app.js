                ...
                <div className="overflow-auto flex-1">
                    <table className="w-full text-left text-sm text-slate-300 whitespace-nowrap">
                        <thead className="bg-slate-900 text-white uppercase text-xs sticky top-0 z-10">
                            <tr>
                                <th className="p-3 sticky left-0 bg-slate-900">Joueur</th>
                                <th className="p-3">MJ</th>
                                <th className="p-3">MIN</th>
                                {viewMode === 'classic' ? (
                                    <>
                                        <th className="p-3 text-orange-400">PTS</th>
                                        <th className="p-3">TIR</th>
                                        <th className="p-3">%</th>
                                    ...
                                    <>
                                        ...
                                        <th className="p-3 text-purple-400">ORtg</th>
                                        <th className="p-3 text-red-400">DRtg</th>
                                        <th className="p-3 text-yellow-400">Net</th>
                                        <th className="p-3 text-cyan-400">PIE</th>
                                    </>
                                )}
                            </tr>
                        </thead>
                                ...
                                <tr key={p.info.id} onClick={() => setSelectedPlayer(p)} className="hover:bg-slate-700/50 cursor-pointer">
                                    ...
                                    {viewMode === 'classic' ? (
                                        ...
                                        <>
                                            ...
                                            <td className="p-3 font-bold text-cyan-400">{p.avg.PIE}%</td>
                                        </>
                                    )}
                                </tr>
                            ))}
                                ...
                                <table className="w-full text-left text-xs text-slate-300">
                                    <thead className="bg-slate-700 text-white sticky top-0">
                                        <tr>
                                            ...
                                            <th className="p-2">eFG%</th>
                                            <th className="p-2">ORtg</th>
                                            <th className="p-2">DRtg</th>
                                            <th className="p-2">PIE</th>
                                            <th className="p-2">ÉVAL</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-700">
                                        {selectedPlayer.logs.map((log, i) => (
                                            <tr key={i} className={log.pts === selectedPlayer.records.pts ? 'bg-yellow-900/20' : ''}>
                                                <td className="p-2">{log.date}</td>
                                                <td className="p-2 font-bold">{log.opponent}</td>
                                                <td className="p-2">{log.min}</td>
                                                <td className="p-2 font-bold text-white">{log.pts}{log.pts === selectedPlayer.records.pts && <span className="ml-1 text-yellow-400">🏆</span>}</td>
                                                <td className="p-2">{log.reb}</td>
                                                <td className="p-2">{log.ast}</td>
                                                <td className="p-2">{log.eFG}%</td>
                                                <td className="p-2 text-purple-400">{log.ORtg}</td>
                                                <td className="p-2 text-red-400">{log.DRtg}</td>
                                                <td className="p-2 text-cyan-400">{log.PIE}%</td>
                                                <td className="p-2">{log.eff}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                    ...
                    <div className="h-64">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={teamTrendsData}>
                                ...
                                <YAxis stroke="#94a3b8" fontSize={10} />
                                <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: 'none' }} />
                                <Bar dataKey="NetRtg" name="Net Rating" fill="#f97316">
                                    {teamTrendsData.map((entry, index) => (
                                        <rect key={`cell-${index}`} fill={entry.NetRtg >= 0 ? '#22c55e' : '#ef4444'} />
                                    ))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
