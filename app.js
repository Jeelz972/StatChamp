...
function buildScoreHistoryFromActions(actions, players) {
  ...
  let home = 0,
    away = 0;
  const history = [{ time: 600, q: 1, home: 0, away: 0 }];
  ...
  sorted.forEach((a) => {
    ...
    if (pts > 0) {
      ...
      history.push({ time: a.time || 0, q: a.q || 1, home, away });
    }
  });
  return history;
}
...
function MomentumChart({ scoreHistory, actions, players: matchPlayers }) {
  ...
  }, [scoreHistory, actions, matchPlayers]);
...
function GameDetailsModal({ game, isOpen, onClose, players, isAdmin }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Vs ${game.opponent}`} size="max-w-6xl">
      ...
      {/* A1 — ROTATION CHART */}
      {game.actions?.length > 0 && game.starters && window.RotationChart && (
    ...
    <div className="space-y-6">
      <Card className="p-6 border-l-4 border-orange-500">
        <h3 className="text-lg font-bold text-white mb-4">
          <Icon path={Icons.Layers} /> Phases
        </h3>
        <div className="space-y-3">
          {phases.map((ph, i) => (
            <div
              key={ph.id}
              className="flex items-center gap-3 bg-slate-900 p-3 rounded border border-slate-700"
            >
              ...
              <button
                onClick={() => {
                  if (phases.length > 1 && confirm('Supprimer ?'))
                    onUpdatePhases(phases.filter((p) => p.id !== ph.id));
                }}
            ...
            <input
              ...
              onKeyPress={(e) => {
                if (e.key === 'Enter' && newPhaseName.trim()) {
                  onUpdatePhases([
                    ...phases,
                    { id: `phase_${generateId()}`, name: newPhaseName.trim() },
                  ]);
                  setNewPhaseName('');
                }
              }}
      ...
      <Card className="p-6 border-l-4 border-purple-500">
        ...
        <textarea
          className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-xs text-white font-mono h-32 mb-2"
          value={localConfig}
        ...
        <Button
          onClick={() => {
            try {
              setFirebaseConfig(JSON.parse(localConfig));
              alert('OK');
            } catch (e) {
              alert('Erreur JSON');
            }
      ...
      <div className="grid gap-3">
        {players.map((p, i) => (
          <Card key={p.id} className="p-3 flex gap-3 items-center">
            <input
              className="w-12 h-12 bg-slate-700 rounded-full text-center font-bold outline-none"
              value={p.number}
              onChange={(e) => {
                const np = [...players];
                np[i].number = parseInt(e.target.value) || 0;
                onUpdatePlayers(np);
              }}
            />
            <input
            ...
            />
            <input
              className="w-16 bg-transparent text-slate-400 text-sm outline-none"
              value={p.pos}
              onChange={(e) => {
                const np = [...players];
                np[i].pos = e.target.value;
                onUpdatePlayers(np);
              }}
            />
            <select
...
function App() {
  ...
  useEffect(() => {
    ...
    if (PRECONFIGURED_FIREBASE.apiKey && !savedFbConfig) setFirebaseConfig(PRECONFIGURED_FIREBASE);
      ...
      try {
        setFirebaseConfig(JSON.parse(savedFbConfig));
      } catch (e) {}
    ...
    if (firebaseConfig && window.firebase) {
      try {
        ...
        database
          ...
          .doc('roster')
          .onSnapshot((doc) => {
            if (doc.exists && doc.data().list) setPlayers(doc.data().list);
  ...
  useEffect(() => {
    if (firebaseConfig)
      localStorage.setItem('basket_firebase_config', JSON.stringify(firebaseConfig));
  }, [firebaseConfig]);
  ...
  const handleSaveGame = (gameState) => {
    ...
    const newGamesList = games.some((g) => g.id === gameId)
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex-1 p-4 overflow-y-auto" style={{ zIndex: 10 }}>
          {view === 'global_stats' && window.GlobalStats && (
            <window.GlobalStats players={players} games={games} phases={phases} isAdmin={isAdmin} />
          )}
          {view === 'history' && (
            <History
              games={games}
          ...
          {view === 'settings' && isAdmin && (
            <Settings
              players={players}
              onUpdatePlayers={handleSettingsUpdate}
              phases={phases}
